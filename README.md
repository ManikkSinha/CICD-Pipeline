...
4) Name the repository CICD-Pipeline-repo.
...
gcloud source repos clone CICD-Pipeline-repo
12) The previous command created an empty folder called CICD-Pipeline-repo. Change to that folder:
cd CICD-Pipeline-repo
...
2) Select the gcp-course > CICD-Pipeline-repo folder in the explorer tree on the left.
3) Click on CICD-Pipeline-repo
4) On the File menu, click New File
5) Paste the following code into the file you just created:
https://github.com/logicopslab/CICD-Pipeline/blob/main/main.py
6) To save your changes. Press CTRL + S, and name the file as main.py.
7) Click on SAVE
8) Click on the CICD-Pipeline-repo folder.
...
https://github.com/logicopslab/CICD-Pipeline/blob/main/layout.html
...
https://github.com/logicopslab/CICD-Pipeline/blob/main/index.html
...
15) In the CICD-Pipeline-repo folder (not the templates folder), create a New File and add the following to that file and save it as requirements.txt:
https://github.com/logicopslab/CICD-Pipeline/blob/main/requirements.txt
...
cd ~/gcp-course/CICD-Pipeline-repo
git add --all
...
1) In the Cloud Shell Code Editor, expand the gcp-course/CICD-Pipeline-repo folder. With the CICD-Pipeline-repo folder selected, on the File menu, click New File and name the new file Dockerfile.
The file Dockerfile is used to define how the container is built.
https://github.com/logicopslab/CICD-Pipeline/blob/main/Dockerfile
...
gcloud builds submit --tag gcr.io/$DEVSHELL_PROJECT_ID/CICD-Pipeline-image:v0.1 .
...
<img width="438" alt="image" src="https://github.com/logicopslab/CICD-Pipeline/assets/82759985/5cbedb42-bfdf-42fb-a7da-ca7298a69caf">
...
1) On the Navigation menu (Navigation menu icon), click Container Registry. At this point, you should have a folder named CICD-Pipeline-image with at least one container in it.
...
5) Name the trigger CICD-Pipeline-trigger.
6) Select your CICD-Pipeline-repo Git repository under repository dropdown.
...
13) Return to the Container Registry service. You should see a new folder, CICD-Pipeline-repo, with a new image in it.
14) Return to the Cloud Shell Code Editor. Find the file main.py in the gcp-course/CICD-Pipeline-repo folder.
...
1) When the build completes, click on it to see its details. Under Execution Details, copy the Image link, format should be gcr.io/qwiklabs-gcp-00-f23112/CICD-Pipeline-repoxx34345xx.
